...
$(document).ready(function() {
    ...
    WeightedGraph.prototype.buildVisual = function() {
        ...
        // build the label
        this.myLabel = this.paper.text(this.x, this.y + (45 + 13)*2, this.type + " " + this.name);
        this.myLabel.attr({
            "opacity": 0,
            "font-family": "times",
    ...
    WeightedGraph.prototype.move = function(newX, newY) {
        ...
        var _t = this;
        setTimeout(function() {
            var time = _t.VH.getAnimTime(500);
            _t.myLabel.animate({
    ...
    WeightedGraph.prototype.createNode = function(delay, time) {
        this.getNextPos();
    ...
    WeightedGraph.prototype.highlightNode = function(node, delay, time, color) {
        var anim = Raphael.animation({
            stroke: color
        }, time);
    ...
    WeightedGraph.prototype.createEdge = function(fromNode, toNode, weight, delay, time, color) {
        console.log("createEdge(" + fromNode.id + "," + toNode.id + "," + weight + ")");
        // build the animation
        var anim = Raphael.animation({
            opacity: 1,
            "stroke": color || "#4b4b4b"
    ...
    WeightedGraph.prototype.highlightEdge = function(fromNodeID, toNodeID, delay, time, color) {
    ...
    WeightedGraph.prototype.erase = function() {
        ...
        console.log("WeightedGraph erase()");
    ...
    WeightedGraph.prototype.update = function(action, originADT) {
        ...
        switch (split[0]) {
            ...
            case "clear":
                ...
                break;
            default:
                console.log("Unknown action for WeightedGraphs: " + action);
                ...
                for (var fromNodeID = 0; fromNodeID < graphVal.length; fromNodeID++) {
                    ...
                    for (var x = 0; x < toNodes.length; x++) {
                        ...
                        if (!this.hasDrawnEdge(fromNodeID, toNodeID)) {
                            ...
                            this.createEdge(f, t, weight, delay, time);
                        }
                    }
                }
            }
    ...
    // highlight fromNode and toNode, then create an edge with the given weight between them and fade it in
    WeightedGraph.prototype.AddEdge = function(fromNodeID, toNodeID, weight) {
        console.log("VH AddEdge ( " + fromNodeID + " , " + toNodeID + "," + weight + " )");
        var f = this.nodes[fromNodeID];
        var t = this.nodes[toNodeID];
        ...
        this.createEdge(f, t, weight, delay3, time3, "green");
    ...
    // highlight fromNode and toNode, then highlight the edge between them
    // change the weight of the edge then unhighlight all
    WeightedGraph.prototype.SetWeight = function(fromNodeID, toNodeID, newWeight) {
        console.log("VH setWeight ( " + fromNodeID + " , " + toNodeID + "," + newWeight + " )");
        var f = this.nodes[fromNodeID];
        var t = this.nodes[toNodeID];
        var edge = this.getEdge(fromNodeID,toNodeID);
        ...
        // highlight edge from (fromNode,toNode)
        var delay3 = this.VH.setDelay(250);
        var time3 = this.VH.getAnimTime(250);

        this.VH.setDelay(500);

        // highlight current edge weight in red
        var delay4 = this.VH.setDelay(250);
        var time4 = this.VH.getAnimTime(250);
        var anim4 = Raphael.animation({"stroke": "red"},time4);

        this.VH.setDelay(500);

        // fadeout the current edge weight
        var delay5 = this.VH.setDelay(250);
        var time5 = this.VH.getAnimTime(250);
        var anim5 = Raphael.animation({"opacity": 0},time5);

        // remove the current edge weight and build the new weight
        this.VH.setDelay(250);
        var delay6 = this.VH.setDelay(10);
        this.VH.setDelay(250);

        // fade in the new edgeweight
        var delay7 = this.VH.setDelay(500);
        var time7 = this.VH.getAnimTime(500);
        var anim7 = Raphael.animation({"opacity":1},time7);

        this.VH.setDelay(500);

        // unhighlight everything
        var delay8 = this.VH.setDelay(500);
        var time8 = this.VH.getAnimTime(500);

        this.highlightNode(f, delay1, time1, "green");
        this.highlightNode(t, delay2, time2, "green");
        this.highlightEdge(fromNodeID,toNodeID,delay3,time3,"green");
        edge.weight.animate(anim4.delay(delay4));
        edge.weight.animate(anim5.delay(delay5));

        //var _t = this;
        var _edge = edge;
        setTimeout(function() {
            _edge.weight.attr('text',(newWeight.toString()));
        }, delay6);

        edge.weight.animate(anim7.delay(delay7));
        this.highlightNode(f, delay8, time8, "#4b4b4b");
        this.highlightNode(t, delay8, time8, "#4b4b4b");
        this.highlightEdge(fromNodeID,toNodeID,delay8,time8,"#4b4b4b");
    };

    // highlight fromNode and toNode, then highlight the edge between them
    // then unhighlight all
    ...
    WeightedGraph.prototype.HasEdge = function(fromNodeID, toNodeID) {
        ...
        // highlight edge or make nodes red
        var delay3 = this.VH.setDelay(250);
        var time3 = this.VH.getAnimTime(250);
    ...
    WeightedGraph.prototype.GetDegree = function(fromNodeID) {
        ...
        for (var i=0; i<graphVal.length; i++) {
            if (i === fromNodeID) {
                var fromVal = graphVal[i]; // [[toVal,weight], [toVal,weight]]
                for (var j=0; j<fromVal.length;j++) {
                    var toPair = fromVal[j];
                    neighborVals.push((toPair[0]));
                }
            }
        }
        ...
        for (var i=0; i<neighborVals.length; i++) {
            ...
            var toID = neighborVals[i];
            var t = this.nodes[toID];
            this.highlightNode(t,delay2,time2,"green");
        }
    ...
    WeightedGraph.prototype.GetInDegree = function(toNodeID) {
        ...
        var graphVal = this.value[0]; // [[[to,weight],[to,weight]],[[to,weight],[to,weight]]...]
        for (var i=0; i<graphVal.length; i++) {
            var fromVal = graphVal[i]; // [to,weight]
            for (var j=0; j<fromVal.length;j++) {
                var toPair = fromVal[j];
                if (toNodeID === toPair[0]) {
                    neighborVals.push(i);
                }
            }
        }
    ...
    WeightedGraph.prototype.Clear = function() {
        ...
        for (var j = 0; j < this.wedges.length; j++) {
            ...
            if (typeof(currentWedge) !== "undefined") {
                ...
                currentWedge.weight.animate(anim1.delay(delay1));
                if (this.isDirected) {
